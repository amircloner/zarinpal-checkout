...
/**
 * Validate Payment with ExtraDetail from Authority.
 * @param  {number} Amount
 * @param  {String} Authority
 */
ZarinPal.prototype.PaymentVerificationExtra = function(input) {
	var self = this;
	var params = {
		MerchantID: self.merchant,
		Amount: input.Amount,
	...
	var promise = new Promise(function (resolve, reject) {
		self.request(self.url, config.API.PVX, 'POST', params).then(function (data) {
			resolve({
				status: data.Status,
				RefID: data.RefID,
				CardPanMask: data.ExtraDetail ? data.ExtraDetail.Transaction.CardPanMask : '',
				CardPanHash: data.ExtraDetail ? data.ExtraDetail.Transaction.CardPanHash : ''
			});
		}).catch(function (err) {
			reject(err);
		});
